 ...
 */
public class Venta1 extends javax.swing.JDialog {
    ...
    public Venta1(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    public Venta1(java.awt.Frame parent, boolean modal, int cod) {
        super(parent, modal);
        initComponents();
        this.setLocationRelativeTo(null);
        establecerFecha();
...
//        String sql = "select * from usuarios where cod_usu='1101715876'";
    ...
    public void cargarDatos() {
        String datosCli = "select * from clientes where ced_cli = (select ced_cli_f from cabecera_factura where num_fac =" + codFac + ")";
        String total = "select total from cabecera_factura where num_fac="+codFac;
//        String sql = "select * from clientes where ced_cli='172438393'";
        cn = cc.conectar();
        try {
            Statement st = cn.createStatement();
            ResultSet rsCli = st.executeQuery(datosCli);
            
            while (rsCli.next()) {
                jTextField_CedCli.setText(rsCli.getString("CED_CLI"));
                jTextField_NomCli.setText(rsCli.getString("NOM_CLI"));
                jTextField_ApeCli.setText(rsCli.getString("APE_CLI"));
                jTextField_TelCli.setText(rsCli.getString("TEL_CLI"));
                jTextField_DirCli.setText(rsCli.getString("DIR_CLI"));
                jTextField_CedCli.setEditable(false);
                habilitarEdicionCamposCliente(false);
                jButton_Seleccionar.setEnabled(true);
            }
            ResultSet rsTot = st.executeQuery(total);
            while(rsTot.next()){
                jTextField_Total.setText(rsTot.getString("total"));
            }
            cn.close();

        } catch (SQLException ex) {
            JOptionPane.showMessageDialog(null, "b" + ex);
            try {
                cn.close();
            } catch (SQLException ex1) {
                JOptionPane.showMessageDialog(null, "Error de conexión\n" + ex1);
    ...
    public void modeloTablaCarrito() {
        ...
        String sql = "select s.cod_ser cod, s.nom_ser nom, d.cant can, s.cos_ser cos,(s.cos_ser*d.cant) val,f.total tot "
                + "from detalle_factura d,servicios s, cabecera_factura f "
                + "where d.cod_ser_uso=s.cod_ser "
                + "and d.num_fac_per=f.num_fac "
                + "and d.num_fac_per=" + codFac;
        cn = cc.conectar();
        try {
            Statement st = cn.createStatement();
            ResultSet rs = st.executeQuery(sql);
            while (rs.next()) {
                ...
                datos[2] = rs.getString("can");
                datos[3] = rs.getString("cos");
                datos[4] = rs.getString("val");
                jTextField_Total.setText(rs.getString("tot"));
            ...
            JOptionPane.showMessageDialog(null, "a" + ex);
        }
    ...
    public boolean ActTabla(int col, int fil) {
        ...
        if (col == 2) {
            sql = "update detalle_factura "
                    + "set cant =" + Integer.valueOf(jTable_CarritoCompra.getValueAt(fil, col).toString()) + " "
                    + "where num_fac_per=" + codFac;
            aux = true;
        }
...
//        deshabilitarCancelar();
    ...
    private void jButton_CobrarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton_CobrarActionPerformed
        int preg = JOptionPane.showConfirmDialog(this, "Realizar Paga?...", "Facturando...", JOptionPane.YES_OPTION);
        if (preg == 0) {
            //cancelar();
        }
...
//        confirmarCancelarVenta();
        int confirm = JOptionPane.showConfirmDialog(null, "¿Desea volver?", "Cancelar venta", JOptionPane.YES_OPTION);
        if (confirm == 0) {
            this.dispose();
        }
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            ...
            public void run() {
                Venta1 dialog = new Venta1(new javax.swing.JFrame(), true);
